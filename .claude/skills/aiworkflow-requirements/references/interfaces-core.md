...
RAGパターンでは、Repository呼び出しはResult型を返す。呼び出し元はsuccessプロパティで成功/失敗を判定し、失敗時はerr関数でエラーを上位に伝播する。成功時はdataプロパティからデータを取得して処理を継続する。

| ステップ | 処理内容              | 詳細                                               |
| -------- | --------------------- | -------------------------------------------------- |
| 1        | Repository呼び出し    | findByIdなどのメソッドでResult型を取得             |
| 2        | 成功/失敗の判定       | result.successプロパティで判定                     |
| 3a       | 失敗時の処理          | err(result.error)でエラーを上位に伝播して終了      |
| 3b       | 成功時の処理          | result.dataからエンティティを取得して処理を継続    |

---

## Result型

成功・失敗を明示的に表現する型。例外を使わないエラーハンドリングに使用（Resultベースの明示的エラーハンドリング）。
